...
- **`ReplaceFuncFirst(re *regexp.Regexp, target string, fn func(group, match string) string) string`** — the first-match-only callback variant that completes the replace family's matrix (static/func × all/first): `ReplaceFunc`'s callback adapter passed through the shared `replaceNamed` engine with `limit=1`, exactly as `ReplaceFirst` did for `Replace`. It mechanically inherits the family's contracts: within the first match, every participating named span reaches `fn` left to right (return the match verbatim to keep a group), overlapping spans resolve outermost-first with `fn` never invoked for a suppressed inner group, every later match and all outside text passes through byte-for-byte, `target` is returned unchanged on no match with `fn` never called, and a nil `fn` panics on the first match per stdlib `Regexp.ReplaceAllStringFunc`. With this the replace family is closed at four functions — the policy is recorded in CONTRIBUTING's "Free functions serve the 80% case"; anything needing more knobs (limits beyond 1/-1, per-group callbacks, byte offsets) belongs on `Decoder` or a future options-taking variant. Additive, non-breaking. ([#198](https://github.com/Jecoms/regextra/issues/198))
- **`(e *Encoder[T]).EncodeStrict(v T) (string, error)` — opt-in strict encode that verifies each encoded value re-matches its group's sub-pattern.** Promotes the round-trip contract's documented "future option" to the API: `EncodeStrict` walks the same derived plan as `Encode` but re-matches every rendered field value against a fully anchored (`\A(?:sub)\z`) compile of its capture group's sub-pattern, retained per segment at derivation time (eager, so encoders keep zero shared mutable state after construction). When the sub-pattern contains a zero-width assertion (`\b`, `\B`, a multiline `^`/`$`, `\A`, `\z`), the matcher bakes in one rune of the adjacent literal segments so edge assertions evaluate against the characters the emitted string actually places there rather than against the bare value in isolation. A miss returns the familiar `errors.As`-able `*EncodeError` (Field/Group/Type populated) whose cause wraps a new `errors.Is`-checkable `ErrValueMismatch` sentinel naming the rendered value and the sub-pattern, under the `regextra.Encoder.EncodeStrict:` entrypoint prefix. The check verifies the contract's stated per-group condition, not a full re-decode — delimiter-collision and adjacent-capture ambiguities remain out of scope, as documented. `Encode` stays fast/lenient, and on values that pass, `EncodeStrict` output is byte-identical to `Encode`'s. Method form (no construction-time toggle) mirrors the package's named-variant surface (`One`/`All`, `Replace`/`ReplaceFirst`). Additive, non-breaking. ([#200](https://github.com/Jecoms/regextra/issues/200))
- **`regex:",inline"` — opt-in embedded-struct field promotion in the decode and encode plans.** An embedded struct (or `*struct`) field tagged with the new `inline` lone-token flag (claiming the second reserved flag slot; `required` took the first) has its exported fields promoted into the plan as if declared on the outer struct — tags, name fallback, `default=`, `layout=`, and `required` all apply, recursively through nested `inline` tags, with pointer-embedding cycles terminating. Precedence follows `encoding/json`: a shallower field bound to a group shadows a deeper promoted one; promoted fields tying at the shallowest depth resolve to a sole explicitly tagged binding (json's tagged-beats-untagged tiebreak), while an unresolved tie — no tagged binding, or several — is rejected by `Compile` (`ErrInvalidStruct`) and dropped by the lenient `Unmarshal`/`UnmarshalAll` path. `Compile` also rejects `inline` on a non-embedded field and a group name combined with `inline`. The encoder mirrors promotion with the same shadowing and tiebreak rules: decode allocates a nil embedded pointer on the way down, while `Encode` through a nil embedded pointer fails with an `EncodeError` (an absent destination can be created; an absent source cannot be read). Untagged embedded fields stay non-promoted for all of v2 — promotion stays opt-in; default-on `encoding/json` parity is a v3 candidate ([#206](https://github.com/Jecoms/regextra/issues/206)). Additive, non-breaking (new tag-grammar flag per the documented forward-compat rules). ([#201](https://github.com/Jecoms/regextra/issues/201))
...
- **Composition idioms documented in the package doc.** New "When the standard library already does it" section records the two deliberate non-features: template-style `$name` replacement (that is stdlib `Expand`/`ReplaceAllString`; regextra's `Replace` family substitutes matched spans in place instead) and streaming input (compose a `bufio.Scanner` with `Decoder.One`, shown in the new runnable `ExampleDecoder_One_scanner`). `Replace`'s godoc cross-references the section. Behavior unchanged. ([#199](https://github.com/Jecoms/regextra/issues/199), [#213](https://github.com/Jecoms/regextra/pull/213))

### Performance

- **Per-field converter funcs compiled into the decode plan.** `setFieldValue` re-derived the type dispatch on every decoded field — pointer check, two `RegexUnmarshaler` probes, the `time.Time`/`time.Duration` type switch, two `TextUnmarshaler` probes, then the kind switch. The plan build (shared by `Compile` and the cached `Unmarshal`/`UnmarshalAll` path) now resolves a `func(field reflect.Value, value string) error` converter per field from the field's `reflect.Type` and parsed tag options — the `encoding/json` cached-`encoderFunc` precedent — collapsing the per-decode dispatch to one indirect call. Steady-state decode improves across the board with allocs/op unchanged everywhere (2/op simple-struct decode preserved): `DecoderOne/simple` **−24% ns** (272→206 ns), `withTime` **−51%** (745→366 ns — the hoisted interface probes were most of the time-field dispatch), `manyFields` −39%, `Unmarshal/simpleThreeField` −18%, `Unmarshal/wideStruct` −39%, `UnmarshalAll/realisticLogCorpus` −25%, `DecoderAll/matches100` −11%, `DecoderIter/fullIteration` −14% (Apple M4, benchstat n=10, p=0.000). The cost moves to the cold, cached plan build: `Compile/simpleStruct` +7.8% ns, `CompilePlanOnly` +26%, `Compile/timeFieldWithLayout` +61%, and +1 alloc on plans whose converter closures capture state — a sized numeric field capturing its bit width (`CompilePlanOnly` 10→11, `Compile/simpleStruct` 44→45 via their `int` field) or a time field capturing its resolved `layout=` (`timeFieldWithLayout` 23→24) — while capture-free all-string plans (`fields1/10/20`, `wideStruct`) hold flat; paid once per (pattern, type) and justified against the per-decode win per CONTRIBUTING's regression rule. The already-single-probe `RegexUnmarshaler` dispatch paths hold within ±4% ns. Interface-typed fields (e.g. a field declared as `RegexUnmarshaler` or `encoding.TextUnmarshaler`) keep the per-call dynamic dispatch via `setFieldValue`, since their conversion depends on the value the field holds at decode time; unsupported field types (nested struct, slice, map, …) still error at decode time rather than plan-build time, preserving the lenient `Unmarshal` posture; error text is byte-identical. No public API, error, or semantics change. ([#204](https://github.com/Jecoms/regextra/issues/204))
...
**Module path.** v2 lives at the `/v2` import path per Go's module-versioning rules — `go get github.com/jecoms/regextra/v2@latest` and `import "github.com/jecoms/regextra/v2"`. The v1 line is frozen at the `v1.x` tags.
...
- **`ReplaceFunc(re *regexp.Regexp, target string, fn func(group, match string) string) string`** — a callback analogue of `Replace` whose replacement depends on the matched value (redaction, normalization), filling the gap left by `Replace`'s static `map[string]string`. Every participating named group reaches `fn`; return the match verbatim to leave a group unchanged. It shares `Replace`'s engine (the same span collect → stable-sort → outermost-wins apply loop, now factored into an unexported `replaceNamed` helper), so it inherits the same tie-break and no-match contract: on no match the target is returned unchanged and `fn` is never called; `fn` is never invoked for an inner group suppressed by an enclosing outermost span. A nil `fn` panics on the first match, mirroring stdlib `Regexp.ReplaceAllStringFunc`. Additive, non-breaking. ([#117](https://github.com/Jecoms/regextra/issues/117))
- **`(d *Decoder[T]).Regexp() *regexp.Regexp`** — exposes the compiled pattern the `Decoder[T]` already holds, so callers can reuse it for their own match-finding (`FindAllIndex`, custom iteration) without recompiling. Sits beside the existing `Pattern()` accessor; adds no state or compilation. The doc comment warns callers not to mutate shared state on the returned value (e.g. `(*regexp.Regexp).Longest()`). Additive, non-breaking. ([#153](https://github.com/Jecoms/regextra/issues/153))
- **`encoding.TextUnmarshaler` support in field conversion.** Any destination field whose type (or pointer-to-type) implements `encoding.TextUnmarshaler` — `netip.Addr`, `math/big.Int`, `log/slog.Level`, `github.com/google/uuid.UUID`, etc. — now unmarshals out of the box by calling its `UnmarshalText` with the matched value, without a `RegexUnmarshaler` wrapper. Dispatch precedence is `RegexUnmarshaler` → `time.Time`/`time.Duration` special-case → `TextUnmarshaler` → built-in `Kind` switch; the `TextUnmarshaler` check sits *after* the time special-case deliberately, so `time.Time` (which satisfies `TextUnmarshaler` with an RFC3339-only `UnmarshalText`) keeps its multi-layout `parseTime` fallback and the `layout=` tag option. **(Breaking.)** Inserting `TextUnmarshaler` ahead of the `Kind` switch is a public-contract change that cannot be tightened later without a v3 bump: a named primitive type that also implements `TextUnmarshaler` (e.g. `type Level int` with an `UnmarshalText`) now decodes via its `UnmarshalText` instead of the built-in kind conversion it previously received. `time.Time`/`time.Duration` behavior is unchanged. ([#119](https://github.com/Jecoms/regextra/issues/119))
...
- Removed an unreachable value-receiver dispatch branch in the unmarshal type-switch. Internal refactor with no observable behavior change. ([#125](https://github.com/Jecoms/regextra/pull/125))
...
- **`Unmarshal` / `UnmarshalAll` now cache the decode plan per (pattern, struct type) pair.** The free functions previously rebuilt the reflect decode plan on every call — exactly the work `Decoder[T]` computes once at `Compile`. They now consult an internal process-lifetime cache (a `sync.Map` keyed by the regexp's source pattern plus the destination `reflect.Type`, the `encoding/json` field-cache precedent), so only the first call for a given pair pays the plan build. Steady-state `Unmarshal` on a simple three-field struct drops **−48% ns/op and −82% allocs/op** (558→290 ns, 11→2 allocs) and a wide struct **−70% ns / −96% allocs** (3.55→1.08 µs, 48→2 allocs); `UnmarshalAll/singleMatch` −42% ns / −64% allocs (Apple M4, benchstat n=10, p=0.000). `Compile`/`Decoder` benchmarks are unchanged. The cache is never evicted: entries are small and do not retain the `*regexp.Regexp`, and a workload decoding unboundedly many distinct patterns already pays per-call regexp compilation that dwarfs plan retention. Caching is sound because the lenient plan build depends on the regexp only through `SubexpNames`/`SubexpIndex` — pure functions of the pattern string — and the plan is immutable after build. No public API, error, or semantics change. ([#171](https://github.com/Jecoms/regextra/issues/171))
- **Benchmark suite plus allocation cuts across `Replace`, the `Find*` helpers, and `UnmarshalAll`.** Added the package benchmark suite and trimmed off-hot-path allocations in the named-group readers and the replace engine (which also standardized `Replace`'s overlapping-span tie-break on a stable sort). No public API or behavior change. ([#129](https://github.com/Jecoms/regextra/pull/129), fixes [#107](https://github.com/Jecoms/regextra/issues/107))
- **`AllNamedGroups` trims off-hot-path allocations.** Reworked the internal collection to avoid intermediate allocations that don't affect the returned `map[string][]string`. No public API or behavior change. ([#164](https://github.com/Jecoms/regextra/pull/164))
- **Six small confirmed allocation wins across `Encode`, `Compile`/plan build, `FindNamed`/`FindAllNamed`, and `Decoder.Iter`.** Each is an internal one-to-few-line change with a benchstat-confirmed delta and no public API, error, or semantics change: (1) `Encode` reflects on its parameter's address instead of `reflect.New` + `Set` — −1 alloc on every call, −7…−13% ns; (2) `parseFieldTag` walks tag pieces with `strings.Cut` instead of `strings.Split` — −1 alloc per tagged field per plan build (post-#171 this benefits `Compile`/`Encoder` derivation/cache-miss paths); (3) `buildDecodePlan` pre-sizes its plan slice to `NumField()` — `Compile/fields20` −14% ns, −42% B/op; (4) `FindNamed` scans `SubexpNames()` inline instead of materializing an index slice — 2→1 allocs/op, no-match paths 0 allocs; (5) `Decoder.Iter` reuses one zero-reset scratch value across matches instead of allocating a fresh `T` per match — a GC-pressure win (`fullIteration` −48% allocs/op, −20% B/op, ns flat), with regression tests pinning yield-aliasing safety; (6) `FindAllNamed` collects occurrence indices in a stack-backed buffer — −1 alloc/op everywhere (borderline +1.2% ns at 100 matches, best for low-match-count callers). ([#172](https://github.com/Jecoms/regextra/issues/172))
