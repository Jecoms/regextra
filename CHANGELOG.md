...
- **`ValidationError` typed error for `Validate`.** `Validate` now returns an `errors.As`-able `*regextra.ValidationError` whose `Missing []string` field carries the declared-but-absent required group names (in the order passed), so callers can branch on the missing set without parsing `err.Error()`. The error is wrapped with the existing `regextra.Validate:` prefix and its message is unchanged, mirroring the `DecodeError` precedent ([#111](https://github.com/Jecoms/regextra/issues/111)). A directly-constructed empty value (`&ValidationError{}`, which `Validate` itself never produces) renders `no missing named groups` instead of a message with a dangling separator. Additive, non-breaking. ([#151](https://github.com/Jecoms/regextra/issues/151))
